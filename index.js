...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .url-dup-hint { font-size: 12px; color: var(--danger); margin-top: -10px; margin-bottom: 12px; display: none; }
...
<body>
  ...
  <polyline points="12 7 12 12 16 14"></polyline>
    ...
    function customAlert(msg) { el('general-dialog-title').textContent = '提示'; el('general-dialog-content').textContent = msg; el('general-dialog-input').style.display = 'none'; el('general-cancel').style.display = 'none'; el('general-confirm').textContent = '确定'; const thirdBtn = el('general-dialog').querySelector('.btn-third'); if (thirdBtn) thirdBtn.style.display = 'none'; showDialog('general-dialog'); const btn = el('general-confirm').cloneNode(true); el('general-confirm').parentNode.replaceChild(btn, el('general-confirm')); btn.onclick = () => hideDialog('general-dialog'); }
    function customConfirm(msg, btnOkText='确定', btnCancelText='取消') { return new Promise(resolve => { el('general-dialog-title').textContent = '确认'; el('general-dialog-content').textContent = msg; el('general-dialog-input').style.display = 'none'; el('general-cancel').style.display = 'inline-block'; el('general-cancel').textContent = btnCancelText; el('general-confirm').textContent = btnOkText; const thirdBtn = el('general-dialog').querySelector('.btn-third'); if (thirdBtn) thirdBtn.style.display = 'none'; showDialog('general-dialog'); const ok = el('general-confirm'), cancel = el('general-cancel'); const nOk = ok.cloneNode(true), nCancel = cancel.cloneNode(true); ok.parentNode.replaceChild(nOk, ok); cancel.parentNode.replaceChild(nCancel, cancel); nOk.onclick = () => { hideDialog('general-dialog'); resolve(true); }; nCancel.onclick = () => { hideDialog('general-dialog'); resolve(false); }; }); }
    ...
    function showLinkDialog(url=null) {
        ...
        el('url-dup-hint').style.display = 'none';
    }
    // 实时检测URL重复，输入时给出轻提示
    function checkDuplicateUrl() {
        const hint = el('url-dup-hint');
        const old = el('link-old-url').value;
        const url = el('url-input').value.trim();
        if (!url) { hint.style.display = 'none'; return; }
        // 编辑模式下，原URL不算重复
        const dup = state.links.find(l => l.url === url && l.url !== old);
        if (dup) {
            hint.textContent = '该链接已存在于「' + dup.category + '」分类下';
